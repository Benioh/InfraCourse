"""Distributed harness identical in spirit to L02; spawn N gloo workers and run a worker fn."""
...
def _get_impl():
    ...
    return importlib.import_module(f"{name}.manual_ddp")
...
def parallel_run(world_size, fn_name):
    ...
    try:
        for p in procs:
            ...
            if p.exitcode != 0:
                msg = queue.get_nowait()[1] if not queue.empty() else "no message"
                pytest.fail(f"worker exited {p.exitcode}: {msg}")
        if not queue.empty():
