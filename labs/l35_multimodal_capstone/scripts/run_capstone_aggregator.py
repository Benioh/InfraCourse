...
STAGE_RULES = {
    "env": ["l01_", "l02", "l03_"],
    "training": ["l04_", "l05_", "l06_", "l07_", "l08_", "l09_", "l10_", "l11_", "l12_", "l13_", "l14_", "l15_", "l16_", "l17_", "l18_"],
    "serving": ["l19_", "l20_", "l21_", "l22_", "l23_", "l24_", "l25_", "l26_", "l27_"],
    "rl": ["l28_", "l29", "l30", "l31_", "l32", "l33_", "l34"],
}
...
def write_final_readme(
    ...
    本文件由 L41 Capstone 聚合器生成，来源是 `runs/` 中各课程的最新 run。每一行都应能回到 command/config/metrics/report 证据。
...
3. 把训练、Serving 和 RL 源码改造任务产物加入对应 run artifacts。
"""
...
def write_repro_script(final_dir: Path, evidence_rows: list[dict[str, Any]]) -> None:
    lines = [
        ...
        "set -euo pipefail",
        "",
        "# Generated by L41 Capstone aggregator",
        "make check-env",
        "",
    ]
...
def write_risk_register(final_dir: Path, evidence_rows: list[dict[str, Any]]) -> None:
    ...
    risks = [
        ...
        },
        {
            "risk": "源码改造任务缺证据",
            "count": 4,
            "mitigation": "检查训练、Serving 和 RL source_patch_task.md 产物",
        },
    ]
...
def write_architecture(final_dir: Path) -> None:
    write_text(
        ...
        """flowchart LR
  Env[environment/distributed evidence] --> Train[training/data evidence]
  Train --> Ckpt[checkpoint + tokenizer/processor boundary]
  Ckpt --> Serving[serving evidence]
  Ckpt --> RL[RL evidence]
  Serving --> Metrics[metrics/logs/traces]
  RL --> Metrics
  Metrics --> Capstone[L41 evidence_index + risk_register]
""",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Aggregate course runs into L41 final artifacts"
    )
    ...
    ## 1. 目标
    聚合各课程的真实 run 证据，形成 Capstone 可交付包。
